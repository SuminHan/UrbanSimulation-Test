...
path_file = open('path_sequence.txt', 'w')
...
for _ in range(100000):
    ...
    while True:
        ...
        grid_y = (map.grid_height -  1 - agent_pos[1] / block_height).astype(int)

        if 0 <= grid_x < map.grid_width and 0 <= grid_y < map.grid_height:
            np.add.at(grid_count, (grid_y, grid_x), 1)
            # if len(path_seq) == 0 or path_seq[-1][0] != grid_y or path_seq[-1][1] != grid_x:

            gid = str(grid_y*map.grid_width + grid_x)
            if len(path_seq) == 0 or path_seq[-1] != gid:
                # path_seq.append((grid_y, grid_x))
                path_seq.append(gid)
    ...
    # path_file.write(str(path_seq) + '\n')
    path_file.write(' '.join(path_seq) + '\n')
    path_file.write(' '.join(reversed(path_seq)) + '\n')
    path_file.flush()
